<aside id="backend-sidebar" class="bg-white shadow-sm border-end">
    ...
    <div class="p-3 border-top">
        ...
        <ul class="sidebar-menu list-unstyled m-0">
            ...
            <li class="mb-1">
                <a href="{{ route('back.settings.index') }}" class="text-decoration-none d-flex align-items-center p-3 rounded-lg {{ request()->routeIs('back.settings.*') ? 'bg-primary text-white shadow-sm' : 'text-secondary hover-bg-light' }}" style="transition: all 0.3s ease; border-radius: 8px;">
                    <i class="fas fa-sliders-h me-3 {{ request()->routeIs('back.settings.*') ? 'text-white' : 'text-primary' }}" style="font-size: 1rem; width: 20px; text-align: center;"></i>
                    <span class="font-weight-semibold" style="font-size: 14px;">Settings</span>
                </a>
            </li>
        </ul>
    </div>
