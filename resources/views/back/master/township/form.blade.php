...
<div class="d-flex flex-column flex-lg-row" style="min-height: 100vh;">
    ...
    <main class="flex-grow-1 p-4" style="background-color: #f8f9fa;">
        <div class="mb-4">
            <a href="{{ route('master.township.index') }}" class="text-decoration-none text-muted" style="font-size:13px;">
        ...
        <div class="card border-0 shadow-sm" style="border-radius:12px; max-width:600px;">
            <div class="card-body p-4">
                <form action="{{ $item ? route('master.township.update', $item->township_id) : route('master.township.store') }}"
                      method="POST" enctype="multipart/form-data">
                    @csrf
                    @if($item) @method('PUT') @endif

                    <div class="mb-4">
                        <label class="form-label fw-semibold" style="font-size:13px;">Nama Proyek <span class="text-danger">*</span></label>
                        <input type="text" name="township_name"
                               class="form-control @error('township_name') is-invalid @enderror"
                               style="border-radius:8px; height:44px;"
                               value="{{ old('township_name', $item?->township_name) }}"
                               placeholder="cth: BSD City, Alam Sutera" required>
                        @error('township_name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <a href="{{ route('master.township.index') }}" class="btn btn-outline-secondary px-4" style="border-radius:8px;">Batal</a>
                    </div>
                </form>
            </div>
        </div>
