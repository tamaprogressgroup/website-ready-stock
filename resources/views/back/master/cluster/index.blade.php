...
<div class="d-flex flex-column flex-lg-row" style="min-height: 100vh;">
    ...
    <main class="flex-grow-1 p-4" style="background-color: #f8f9fa;">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h4 class="font-weight-bold mb-0" style="color: #3065A3;">Cluster</h4>
                <p class="text-muted mb-0" style="font-size: 13px;">Kelola data cluster properti</p>
            </div>
            <a href="{{ route('master.cluster.create') }}" class="btn btn-primary btn-sm px-4" style="background:#3065A3; border-color:#3065A3; border-radius:8px;">
                ...
                <table class="table mb-0 align-middle" style="font-size:14px;">
                    <thead style="background:#f0f4f8;">
                        <tr>
                            ...
                            <th class="py-3 text-secondary fw-semibold">Nama Cluster</th>
                    ...
                    <tbody>
                        ...
                        <tr class="border-top">
                            ...
                            <td class="py-3 fw-semibold">{{ $item->cluster_name }}</td>
                            <td class="py-3">
                                @if($item->is_active)
                            ...
                            <td class="py-3">
                                <a href="{{ route('master.cluster.edit', $item->cluster_id) }}" class="btn btn-sm btn-outline-primary me-1" style="border-radius:6px;">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <form action="{{ route('master.cluster.destroy', $item->cluster_id) }}" method="POST" class="d-inline"
                                      onsubmit="return confirm('Hapus cluster ini?')">
                                    @csrf @method('DELETE')
                        ...
                        @empty
                        <tr><td colspan="6" class="text-center text-muted py-4">Belum ada cluster.</td></tr>
                        @endforelse
                    </tbody>
                </table>
                </div>
